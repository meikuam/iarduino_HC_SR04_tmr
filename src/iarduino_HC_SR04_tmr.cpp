...
long	iarduino_HC_SR04_tmr::distance(int8_t t){																//	Параметр:				«t» - текущая температура в °C.
		//	Определяем расстояние:																				//
			long l = IHCSR04TVC.valECHO_DATA[numObj]*50*sqrt(273+t)/1000;										//	Определяем расстояние в сантиметрах: L = длительность в мкс * корень(t+273) / 1000, где длительность = количество прерываний * 50 мкс.
